...
class ProgramPendampingan extends Model
{
   use HasFactory;
   ...
   {
      $this->attributes['scope'] = Str::replace(['<div>', '</div>', '&nbsp;'], '', $val);
   }
   ...
   public function setDocumentsAttribute($val)
   {
      $this->attributes['documents'] = Str::replace(['<div>', '</div>', '<ol>', '<li>', '&nbsp;'], ['<p class="text-dark-5 small mt-3 mb-0">', '</p>', '<ol class="mt-2">', '<li class="small text-dark-5 mt-2">', ''], $val);
   }
}
