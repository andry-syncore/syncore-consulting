...
@section('content')
   <div class="card card-body border-0">
      ...
      @endif
      <form class="row" action="{{ route('program-pendampingan.update', $program) }}" method="post" enctype="multipart/form-data">
         @csrf
         @method('PUT')
         ...
         <div class="col-md-6">
            <div class="mb-3">
               ...
               <input class="form-control @error('name') is-invalid @enderror" id="name" name="name" type="text" placeholder="contoh: Dokumen SOP" value="{{ old('name', $program->name) }}">
               @error('name')
            ...
            <div class="mb-3">
               ...
               <input class="form-control @error('scope') is-invalid @enderror" id="scope" name="scope" type="text" placeholder="Masukan ruang lingkup" value="{{ old('scope', $program->scope) }}">
               @error('scope')
                  <div class="invalid-feedback">{{ $message }}</div>
               @enderror
            </div>
         ...
         <div class="col-12">
            <div class="mb-3">
               ...
               <input id="background" name="background" type="hidden" value="{{ old('background', $program->background) }}">
         ...
         <div class="col-md-6">
            <div class="mb-3">
               <label class="form-label" for="documents">Dokumen Hasil Pendampingan</label>
               <input id="documents" name="documents" type="hidden" value="{{ old('documents', $program->documents) }}">
               <trix-editor class="@error('documents') is-invalid @enderror" input="documents"></trix-editor>
               @error('documents')
                  <div class="invalid-feedback">{{ $message }}</div>
               @enderror
            </div>
